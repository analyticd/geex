...
"""
Unit tests for geex_sqalchemy.py.

Run this from the command line with 'python
geex_sqalchemy_test.py -v'.

Each test will automatically create a geex_test.db in the current
...
class BaseTests(unittest.TestCase):
    test_database_dir = '.'
    test_database_file = 'geex_test.db'
    ...
    # geex_sqlalchemy that has the db, session and metadata
    # variables
    fsqa = None

    # each desired nugget consists of int(NUGID) and
    # str(FILENAME) (including .geex)
    fake_nuggets = ( (1, u'nugid1.geex', 'this is the first nug'),
                     (2, u'nugid2.geex', 'this is the second nug'),
                     (3, u'nugid3.geex', 'this is the third nug'),
                     (4, u'nugid4.geex', 'this is the fourth nug'),
                     (5, u'nugid5.geex', 'this is the fifth nug'),
                     )

    # each fake alias consists of int(NUGID) (which must
    # already exist) and str(ALIAS) (without .geex
    # extension)
...
########################################
    def setUp(self,in_memory=1):
        """
        Deletes 'geex_test.db' if it exists, and creates a
        new Fsqa instance, which will create a new, empty
        'geex_test.db'. Does not populate the database.
        ...
        if in_memory==1:
            ...
            self.fsqa = create_fsqa('geex_test_in_memory.db',
            ...
            if os.path.exists('geex_test_in_memory.db'):
...
class FsqaTests(BaseTests):
    """
    This is the set of test cases for testing geex_sqlalchemy.py
    """
...
########################################
    def test_change_filename_from(self):
        ...
        if not u'nugid1.geex' in get_all_filenames():
            raise 'Problem with populating the db'

        # standard filename change
        nugid = change_filename_from(u'nugid1.geex', u'new_nugid1.geex')
        self.assertEquals(nugid,1)
        self.assertEquals(get_filename(1), u'new_nugid1.geex')
        self.assertEquals(False, u'nugid1.geex' in get_all_filenames())
        self.assertEquals(True, u'new_nugid1' in get_aliases(1))

        # filename with dots in
        nugid = change_filename_from(u'nugid2.geex', u'nugid2.with.dots.geex')
        self.assertEquals(nugid,2)
        self.assertEquals(get_filename(2),'nugid2.with.dots.geex')
        self.assertEquals(False, u'nugid2.geex' in get_all_filenames())
        self.assertEquals(True, u'nugid2.with.dots' in get_aliases(2))

        # neither filename can be none or empty
        self.assertRaises(CantBeNone, change_filename_from, u'nugid1.geex', None)
        self.assertRaises(CantBeNone, change_filename_from, None, u'newfile.geex')
        self.assertRaises(CantBeEmptyString, change_filename_from, u'nugid1.geex', u'')
        self.assertRaises(CantBeEmptyString, change_filename_from, u'', u'newfile.geex')

        # OLD and NEW can't be identical
        self.assertRaises(
            FilenameAlreadyExists,
            change_filename_from, u'nugid1.geex', u'nugid1.geex')

        # the OLD must exist
        self.assertRaises(NonexistentFilename, change_filename_from, u'newfile.geex', u'nugid2.geex')
        self.assertRaises(NonexistentFilename, change_filename_from, u'newfile.geex', u'newfile2.geex')
        
        # the NEW must not exist
        self.assertRaises(FilenameAlreadyExists, change_filename_from, u'nugid3.geex', u'nugid4.geex')
...
########################################
    ...
    def test_put_filename(self):
        ...
        desired_filename1 = 'blah1.geex'
        desired_filename2 = 'blah2.geex'
        ...
        self.assertEquals(None, put_filename(100,'blah1000.geex'))
        
        aberrants = (
            # first input is wrong type
            (ShouldBeInt, put_filename, 'nugid1','blah1000.geex'),
            (ShouldBeUnicode, put_filename_a, 1,'blah1000.geex'),
            ...
            (NonexistentAlias, put_filename_a, 'blah','blah1000.geex'),
        ...
        for nugid,filename,content in fake_nuggets_froox:
            add_nugget(filename,content)
        self.assertRaises( NeedsExtension, add_nugget, 'should_fail.geex','this has the wrong extension')
...
########################################
    def test_get_nugid_from_alias(self):
        ...
        self.assertRaises(NeedsNoExtension,
                          get_nugid_from_alias,'blah.geex')
...
########################################
    def test_get_contents(self):
        ...
        self.assertRaises(NeedsNoExtension,get_contents_a,'blah.geex')
...
########################################
    def test_get_nugid_from_filename(self):
        ...
        self.assertEquals(get_nugid_from_filename(u'nugid1.geex'),1)

        self.assertRaises(NonexistentFilename,
                          get_nugid_from_filename,'nonexistnug.geex')
...
########################################
    def test_add_nugget(self):
        ...
        new_nugid = add_nugget(u'a new nugget.geex','this is the content')
        ...
        self.assertEquals(new_nugid,get_nugid_from_filename(u'a.new.nugget.geex'))

        # the filename has to have a '.geex' extension
        ...
        self.assertRaises(NeedsExtension, add_nugget, 'blah.geex2', 'new content')
        self.assertRaises(NeedsExtension, add_nugget, 'blahgeex', 'new content')

        self.assertRaises(CantBeNone, add_nugget, None, 'new content')
        self.assertRaises(CantBeNone, add_nugget, '.geex', 'new content')
...
########################################
    def test_has_ext(self):
        ...
        """Checking for .geex extension"""

        cases = ( (u'blah.geex', True),
                  (u'.geex',True),
                  (u'a.new.nugget.geex',True),
                  (u'.geex.geex',True),
                  (u'blah',False),
                  (u'blah.muse',False),
                  (u'blah.geex2',False),
                  (u'blahgeex',False),
                  (u'something.geex.blah',False),
                  (u'',False),
                  )
...
########################################
    def test_add_ext(self):
        ...
        """Adding .geex extension"""

        self.assertEquals(add_ext(u'blah'),'blah.geex')
        self.assertEquals(add_ext(u''),'.geex')
        self.assertEquals(add_ext(u'blah.free'),'blah.free.geex')

        self.assertRaises(ShouldBeUnicode,add_ext,100)

        self.assertRaises(NeedsNoExtension,add_ext,'blah.geex')
...
########################################
    def test_remove_ext(self):
        ...
        """Removing .geex extension"""

        self.assertEquals(remove_ext(u'blah.geex'),'blah')
        self.assertEquals(remove_ext(u'geex.geex'),'geex')
        self.assertEquals(remove_ext(u'.geex.geex'),'.geex')
        self.assertEquals(remove_ext(u'blah.geex.geex'),'blah.geex')
        self.assertEquals(remove_ext(u'.geex'),'')

        self.assertRaises(ShouldBeUnicode,remove_ext,100)

        self.assertRaises(NeedsExtension,remove_ext,'blah')
        self.assertRaises(NeedsExtension,remove_ext,'blahgeex')
...
# main
#
# run 'python geex_sqalchemy_test.py -v' at the commandline to test

if __name__ == "__main__":

    forbidden_dirs = ['~/Encrypted/org-mode-repo/geexFiles/']
    ...
    if os.getcwd() in forbidden_dirs:
        raise "Don't run the tests in your main geex docs directory, just in case"
